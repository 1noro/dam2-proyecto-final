...
\begin{document}
    ...
    \lstinputlisting[language=Java, style=javastyle, caption=Implementación de l método \texttt{getThreads()} en el cliente web, label=get:threads:js]{code/getThreads.js}
    ...
    Una vez se hayan rellenado todos los campos obligatorios del formulario, al pulsar sobre el botón de \texttt{Post} se ejecutará su método asociado.

    \lstinputlisting[language=Java, style=javastyle, caption=Implementación de la llamada a (POST) \texttt{/\{slug\}/thread/\{id\}}, label=post:reply:js]{code/postReply.js}
