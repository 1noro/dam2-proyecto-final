...
\begin{document}
    ...
    \begin{figure}[ht]
        ...
        \begin{tikzpicture}[node distance=1.5cm]
            \node (home)  {HOME};
            \node (board) [right=of home] {BOARD};
            \node (thread) [right=of board] {THREAD};
            \draw[->, thick] (home) -- (board);
            \draw[->, thick] (board) -- (thread);
        \end{tikzpicture}
    ...
    \begin{figure}[ht]
        ...
        \label{2chan:home}
        \includegraphics[width=0.9\textwidth]{media/2chan-home.png}
    \end{figure}
    ...
    \begin{figure}[ht]
        ...
        \label{4chan:thread}
        \includegraphics[width=0.9\textwidth]{media/4chan-thread.png}
    \end{figure}
    ...
    Puesto que {\projectname} es un \emph{aplicación web} existen tres escenarios a tratar con respecto a los requisitos de \textit{hardware} y \textit{software}: \textbf{desarrollo}, \textbf{despliegue} e \textbf{instalación por parte del usuario}. Inicialmente se pretende que en cualquier de ellos, estos recursos, sean lo más limitados y gratuitos posibles haciendo que el proyecto pueda ser accesible por cualquiera sin importar la calidad del \textit{hardware} disponible.
    ...
    \begin{figure}[ht]
        ...
        \label{diagram:pert}
        \begin{tikzpicture}[scale=0.80, every node/.style={transform shape}, trinode/.style={circle split, draw, path picture={\draw (path picture bounding box.center)--(path picture bounding box.south);}}]
            % level 0
            \node[trinode] (n1) {1\nodepart{lower} 00\ \ 00};
            % level 1
    ...
    \lstinputlisting[language=Java, style=javastyle, caption=\textit{Request body} en \textit{JSON} de llamada a (POST) \texttt{/x}, label=post:thread:requestbody]{code/post-thread-requestbody.json}
    ...
    \lstinputlisting[language=Python, style=javastyle, caption=Implementación del \textit{DTO} \texttt{ThreadDTO} en el \textit{API}, label=dto:thread]{code/ThreadDTO.py}
    ...
    % Resultados de algunos de los trozos de código que muestro, por ejemplo, algún \textit{JSON} que devuelva el \textit{API}.
    ...
    \lstinputlisting[language=Java, style=javastyle, caption=Implementación del método \texttt{getThreads()} en el cliente web, label=get:threads:js]{code/getThreads.js}

    Esta llamada es interceptada por el \textit{API} y la redirige al siguiente \textit{endpoint}.

    \lstinputlisting[language=Python, style=javastyle, caption=Implementación del \textit{endpoint} (GET) \texttt{/g} en el \textit{API}, label=get:threads:python]{code/get-threads.py}
    ...
    \lstinputlisting[language=Java, style=javastyle, caption=Respuesta en \textit{JSON} de llamar a (GET) \texttt{/g}, label=get:threads:response]{code/get-threads-response.json}
    ...
    \lstinputlisting[language=Java, style=javastyle, caption=Implementación del método \texttt{postReply()} en el cliente web, label=post:reply:js]{code/postReply.js}

    \lstinputlisting[language=Java, style=javastyle, caption=\textit{Request body} que se manda a (POST) \texttt{/\{slug\}/thread/\{id\}}, label=post:reply:requestbody]{code/post-reply-request-body.json}
    ...
    \lstinputlisting[language=Python, style=javastyle, caption=Implementación del \textit{endpoint} (POST) \texttt{/\{slug\}/thread/\{id\}}, label=post:reply:python]{code/post-reply.py}
    ...
    Esta sección es una compilación todos los bloques de código que se han descrito a lo largo del documento, y \textbf{equivale al apartado 7} de la guía del proyecto.
